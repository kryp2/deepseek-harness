...
暂存值至多等待两分钟。它存在的意义是把这次选择与紧随其后的流程所产生的会话衔接起来；若那个流程已经离开而暂存值仍在等待，它就会花费在任何一个碰巧成为当前会话的空白会话上——遗留在其他标签页里的暂存值恰恰是这样做的，它们对同一个会话掀起了一轮 preset 切换风暴，每一次都覆盖了用户真正做出的选择。过期的暂存值回落到部署默认值。选择调用在途时，列表变更也不会为同一选择铸造第二次调用。
